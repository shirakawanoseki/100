...
if __name__ == "__main__":
    ...
    for master_map in master_map_list:
        ...
        for element_dic in master_map:
            #実行初回、もしくは名詞の連続が途切れた場合
            if len(elements_dic_nouns) == 0 and element_dic['pos'] == '名詞':
                elements_dic_nouns.append(element_dic)
                element_dic_prev = element_dic
            #名詞が連続する場合
            elif element_dic['pos'] == '名詞' and element_dic_prev['pos'] == '名詞':
                elements_dic_nouns.append(element_dic)
                element_dic_prev = element_dic
            #名詞が単発で現れて連続しない場合
            elif element_dic['pos'] != '名詞' and 1 == len(elements_dic_nouns):
                elements_dic_nouns = []
            #名詞の連続が途切れた場合
            elif element_dic['pos'] != '名詞' and 2 <= len(elements_dic_nouns):
                for nouns in elements_dic_nouns:
                    print(nouns['surface'],end="")
                print('')
                elements_dic_nouns = []
